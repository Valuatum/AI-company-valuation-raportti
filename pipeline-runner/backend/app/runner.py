...
def _add_to_context(context, stage, output):
    context[WELL_KNOWN.get(stage["order"], _slug(stage["name"]))] = output
...
async def run_stages(run, stages, only=None, from_order=None):
    ...
    for s in stages:
        if not in_scope(s["order"]) and s["order"] in existing:
            ...
            if prev.get("status") in ("ok", "validation_failed"):
                _add_to_context(context, s, _output_value(prev))
    ...
    for s in stages:
        ...
        if res["status"] in ("ok", "validation_failed"):
            _add_to_context(context, s, _output_value(res))
